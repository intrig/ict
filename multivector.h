...
template <typename ValueType, bool is_const_iterator> struct linear_cursor_base;
...
template <typename ValueType, bool is_const_cursor> struct cursor_base {
    ...
    typedef ascending_cursor_base<ValueType, is_const_cursor>
        ascending_cursor_type;
    ...
    friend cursor_base operator-(cursor_base x, difference_type i) {
        return x + (-i);
    }
    friend difference_type operator-(cursor_base &x, cursor_base &y) {
        return x.it_ - y.it_;
    }
    ...
    friend cursor_base operator+(difference_type i, cursor_base x) {
        return x += i;
    }
    ...
    cursor_type begin() {
        return cursor_base(it_->vec_pointer(), it_->begin_ptr());
    }
    ...
    cursor_type end() {
        return cursor_base(it_->vec_pointer(), it_->end_ptr());
    }
    ...
    template <class... Args> cursor_base emplace(Args &&... args) {
        return cursor_base(it_->vec_pointer(),
                           it_->emplace(std::forward<Args>(args)...));
    }
    ...
    cursor_base parent() const {
        return cursor_base(&it_->parent_item()->parent_item()->nodes_,
                           it_->parent_item());
    }
...
struct ascending_cursor_base {
    ...
    bool operator==(const_ascending_cursor_reference it) const {
        return it_ == it.it_;
    }
...
template <typename ValueType> struct item {
    ...
    item(item *parent, const value_type &value)
        : parent(parent), value(value) {}

    template <class... Args>
    item(item *parent, Args &&... args)
    ...
    template <class... Args> item_pointer emplace(Args &&... args) {
    ...
    void promote_last() {
        ...
        std::move(last.nodes_.begin(), last.nodes_.end(),
    ...
    item_pointer end_ptr() {
        return nodes_.empty() ? nullptr : &nodes_.back() + 1;
    }
    const_item_pointer begin_ptr() const {
        return nodes_.empty() ? nullptr : &(nodes_[0]);
    }
    const_item_pointer end_ptr() const {
        return nodes_.empty() ? nullptr : &nodes_.back() + 1;
    }
  ...
  private:
    template <typename T> size_t item_count(T first, T last) const {
        size_t n = 0;
        for (auto i = first; i != last; ++i) {
            if (!i->nodes_.empty())
                n += item_count(i->nodes_.begin(), i->nodes_.end());
            ++n;
        }
        return n;
    }
...
template <typename T> struct init_list_type {
    init_list_type(T v) : d(v) {}
    init_list_type(std::initializer_list<init_list_type<T>> l) : l(l) {}

    template <typename Cursor> void add(Cursor parent) const {
        if (is_list()) {
            if (l.begin()->is_list()) {
                // create a default constructed T and add it if "{{" occurs in
                // initializer list
...
template <typename value_type> struct multivector {
...
template <typename T>
inline std::ostringstream &operator<<(std::ostringstream &ss,
                                      typename item<T>::cursor &a) {
    ss << a->value << " (" << &(*a) << ", " << a->parent << ")";
    return ss;
}
...
template <typename Cursor> Cursor get_root(Cursor start) {
#if 1
    while (!start.is_root())
        start = start.parent();
    return start;
...
template <typename Cursor> Cursor previous(Cursor self) {
...
void recurse(Cursor parent, ActionDown action_down, ActionUp action_up,
             int level = 0) {
    for (auto i = parent.begin(); i != parent.end(); ++i) {
        action_down(i, level);
...
template <typename T> void verify(T parent) {
    recurse(parent, [](T self) {
        ...
        if (!self.empty()) {
            ...
            if (self.begin().it_->parent != &(self.item_ref())) {
                ...
                os << "incorrect first child " << self.begin().it_->parent
                   << ", " << &(*self);
            ...
            if (count != self.size())
                std::runtime_error("incorrect size");
        }
...
template <typename Cursor> inline std::string compact_string(Cursor parent) {
...
template <typename Cursor> inline std::string to_text(Cursor parent) {
    std::ostringstream ss;
    recurse(parent,
...
template <typename T> inline std::string to_text(const multivector<T> &tree) {
...
template <typename Cursor> inline Cursor leaf(Cursor c) {
    if (c.empty())
        return c;
    return --c.end();
}

template <typename Cursor> inline void promote_last(Cursor parent) {
    parent.promote_last();
}
