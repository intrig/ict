...
template <typename T> bool approx(T v, T f, T delta = 0.001) {
...
void ict::bitstring_unit::fromNumber() {
    ...
    }
    {
        ...
        IT_ASSERT_MSG(bs.bit_size() << ":" << sizeof(long),
                      bs.bit_size() == sizeof(long) * 8);
...
void ict::bitstring_unit::toInteger() {
    ...
    }
    {
        ...
        auto c = sub.begin();
        IT_ASSERT_MSG(std::hex << static_cast<int>(c[0]), c[0] == 0x00);
        IT_ASSERT(c[1] == 0x10);
