...
#pragma warning(push)
// This warning appears with std::copy and other std functions called with
// pointers
...
// if the delimiter occurs twice, then it is not a delimeter, it is a single
// char
inline std::vector<std::string> escape_split(const std::string &source,
                                             char del) {
    enum State {
        ReadingName,
        FoundDelimiterOrEsc,
...
inline std::string &replace(std::string &x, std::string const &a,
                            std::string const &b) {
    size_t pos = 0;
...
inline void make_dir(std::string const &path) {
    ...
    if (_mkdir(path.c_str()) == ENOENT)
        IT_PANIC("path not found " << path);
#else
    int ret =
        mkdir(path.c_str(), S_IRWXU | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
    if (!ret && ret != EEXIST)
        IT_PANIC("cannot create directory " << path);
#endif
}

inline int string_case_compare(const char *string1, const char *string2,
                               size_t count) {
#if defined(_MSC_VER)
...
inline bool string_to_int64(int64_t &value, const char *s, int base = 10) {
    ...
    if (errno)
        IT_WARN("string_to_int64 errno: " << strerror(errno));
...
template <typename S> inline std::string extension(const S &filename) {
...
template <typename S> inline void extension(S &filename, const char *new_ext) {
    std::string f = filename.c_str();
    std::string n = new_ext;
    auto index = f.find_last_of('.');
    if (index != std::string::npos)
        f.replace(index, std::string::npos, n);
    filename = f.c_str();
}

// remove leading and trailing spaces
template <typename S> inline S &normalize(S &v) {
    if (v.empty())
        return v;
    if (!isspace(v[0]) && !isspace(v.back()))
        return v;
    auto first = v.begin();
    while (first != v.end() && isspace(*first))
        ++first;
    if (first == v.end()) {
        v = "";
        return v;
    ...
    auto last = --v.end();
    while (isspace(*last) && last != first)
        --last;
    ++last;

    if (first == v.begin() && last == v.end())
        return v;
    v = S(first, last);
    return v;
}
...
// remove all spaces
template <typename S> inline S &squash(S &value) {
    if (value.empty())
        return value;
#if 0 // this line should work, but currently doesn't on clang (cannot deduce
      // 3rd template argument)
 ...
 * </pre>
 */
template <typename T> inline T &xmlize(T &value) {
    if (value.empty())
        return value;
...
inline void write_file(T first, T last, const std::string &name) {
    ...
    if (!s)
        IT_PANIC("Can't open file \"" << name << "\" for writing.");
    s.write(&(*first), last - first);
}

inline void write_file(const std::vector<std::string> &lines,
...
template <typename T, typename String> T to_integer(String s) {
...
inline std::string to_bin_string(InputIterator first, InputIterator last,
                                 size_t bit_size) {
    std::string dest;
...
template <typename T> inline std::string to_hex_string(T first, T last) {
    std::string dest;
...
template <typename T> inline std::string to_json(const std::vector<T> &v) {
    auto os = std::string();
    to_json(os, v);
    return os;
}
